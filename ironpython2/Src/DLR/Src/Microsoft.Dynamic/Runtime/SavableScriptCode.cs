...
namespace AnyPrefix.Microsoft.Scripting {
    ...
    public abstract class SavableScriptCode : ScriptCode {
        ...
        public static void SaveToAssembly(string assemblyName, IMyDictionary<string, object> assemblyAttributes, params SavableScriptCode[] codes) {
            ...
            MyDictionary<Type, List<CodeInfo>> langCtxBuilders = new MyDictionary<Type, List<CodeInfo>>();
            foreach (SavableScriptCode sc in codes) {
                if (!langCtxBuilders.TryGetValue(sc.LanguageContext.GetReleaseType(), out List<CodeInfo> builders)) {
                    langCtxBuilders[sc.LanguageContext.GetReleaseType()] = builders = new List<CodeInfo>();
                }

                MyKeyValuePair<MethodBuilder, Type> compInfo = sc.CompileForSave(tg);
        ...
        protected virtual MyKeyValuePair<MethodBuilder, Type> CompileForSave(TypeGen typeGen) {
            throw new NotSupportedException();
        }

        public override string ToString() {
            return $"ScriptCode '{SourceUnit.Path}' from {LanguageContext.GetReleaseType().Name}";
        }
    }
}
#endif
